...
#driver = webdriver.Chrome(executable_path=r"C:\Users\Jack\Downloads\chromedriver_win32 (1)\chromedriver.exe", options=options)
...
if training==True:
    ...
    for i in range(5):
        time.sleep(1)
        url='https://www.geoguessr.com/game/9oOhIqPK91myZLPJ'
        driver.get(url)
        ...
        if turning==True:
            for i in range(6):
                rotate_canvas()
                time.sleep(0.1)
                driver.find_element(By.CSS_SELECTOR,'#__next > div > div > main > div > div > div.game-layout__panorama > div > div > div > div > div:nth-child(1) > div > div:nth-child(10) > div > canvas').screenshot(f"C:\\Users\\Jack\\Downloads\\data\\test\\FO\\testimage{i}.png")
